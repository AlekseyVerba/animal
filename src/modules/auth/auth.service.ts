...
export class AuthService {
    ...
    async registration({ password, email }: RegistrationDto) {
        ...
        `, [email, hashPassword])).rows[0]
            ...
            await client.query(`
                ...
                RETURNING *
            `, [code, USER_TOKEN_TYPE.Registration, user.uid, Date.now() + (60 * 60 * 1000) + ''])
    ...
    async confirmRegistration({ code, email }: RegistrationConfirmDto) {
        try {
            const user = await this.tokenService.confirmRegistration({ token: code, email });
    ...
    async rememberPassword(email: string): Promise<boolean> {
        ...
        try {
            ...
            await client.query(`
                ...
                RETURNING *
            `, [code, USER_TOKEN_TYPE.RememberPassword, candidate.uid, Date.now() + (60 * 60 * 1000) + ''])
    ...
    async changePassword({ email, password, code }: ChangePasswordDto): Promise<boolean> {
        const client = await this.database.connect()
        try {
            const hashPassword = await hash(password, 8)
