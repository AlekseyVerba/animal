...
export class ChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    async createMessage(message: any) {
        const roomName = this.chatService.getIdFromUids(message.author.uid, message.partner.uid)
    ...
    async deleteMessage(message: any) {
        const roomName = this.chatService.getIdFromUids(message.author_uid, message.partner_uid)
        console.log(`delete message in room - ${roomName}`)
        await this.server.in(roomName).emit('delete_message', message)
    }

    async readMessages(messages: { id: number, author_uid: string, partner_uid: string, is_read: boolean }[]) {
        const roomName = this.chatService.getIdFromUids(messages[0].author_uid, messages[0].partner_uid)
    ...
    async addLike(likeId: number) {
        const like = (await this.database.query(`
            SELECT 
                likes.id as like_id,
                user_uid,
        ...
        const roomName = this.chatService.getIdFromUids(like.author_uid, like.partner_uid)
